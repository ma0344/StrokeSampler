...
- `S=200` / HiRes出力（例: `180x2020 scale10 transparent`）の観測では、低圧側の境界付近で「描画が出始めた最小単位」は **alpha=1/255 が 1px** だった（`alpha_max=1`, `alpha_nonzero_count=1`）。
- 同一条件を複数回（例: 10回）回しても `alpha>0` の有無が揺れない区間が観測できた。少なくとも該当条件では、床は確率的というより **決定的な閾値（区間）** として扱える。
- さらに床直上（`alpha_max=1`, `alpha_nonzero_count=1` の領域）では、`alpha>0` になる **最初の画素座標（first_nonzero_x/y）が trial 間で固定**であることが観測できた。
  - つまり「ノイズでランダムに1pxが出る」のではなく、内部の量子化/丸め/ステップ生成により、同一条件では同一位置に最小単位が現れる。
...
補足（床探索の実務）:
- DotLabの `Export Alpha Presence CSV (PNG, Batch)` を使うと、複数PNGについて `alpha>0` の有無と最初の非ゼロ座標を一覧化できる。境界の安定性（同一Pを複数回回したときに揺れるか）を確認する用途に向く。

## いまやっている検証は「ループから逸脱していないか？」
結論: 逸脱していない。Stage 3（圧力P）を「制御系列（alignedN）」で **境界探索に特化して回している**状態。

今回の進捗（ループの対応づけ）:
- 仮説: N1（初回更新）だけ独立した床/量子化がある。
- 制御系列: StrokeSamplerの `alignedN` を使い、同一幾何（同一位置・同一周期）で N だけを変える。さらに N1 のみを複数trial出力する（同一条件反復）。
- 計測: DotLabで `alpha>0` の有無（presence）と first_nonzero_x/y をCSV化。
- 絞り込み: `P` を刻んで「0→1px（alpha=1/255）」の閾値区間を絞り、かつ trial 間の揺れ/位置の揺れが無いことを確認。

これにより、次の段階（モデル化）に必要な入力が増えた:
- N1の床は確率ではなく決定的に見える（少なくとも該当条件）。
- 床直上の最小出力は 1px/alpha=1/255 で、出現位置も固定。
