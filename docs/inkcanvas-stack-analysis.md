...
## 関連トピック（別文脈）: 線先頭N1 vs 単点（aligned-dot-index）の近似
本ドキュメントは InkCanvas の重ね塗り（累積合成）挙動の切り分けが主題。
一方で、線描画の先頭領域と単点PNGの対応（形状/濃さの近似）をDotLab側で分析する機能も追加している。

- DotLab側: `DotLab/Analysis/LineN1VsDotN1Matcher.cs`
- 引継ぎ: `docs/copilot-session-summary.md` の "Aligned line N1 vs aligned-dot-index N1" 節
