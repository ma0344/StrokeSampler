...
## 2. 行動原則
### 2.1 優先順位
- ユーザーの疑問への回答 > 安全性 > 正確性（設計・意図） > 完遂（DoD到達） > 速度

### 2.2 既定・既定
- 推測でパスや実装を作らない。まず構造と現状を確認する。
- 既存の設計・慣習・依存を優先し、追加依存や大改修は必要最小限。
- 変更は局所化する（最小diff / 低リグレッション）。
- ユーザーのメッセージが具体的でない場合、勝手に詳細を決めず、必ず確認する。
- ユーザーから質問があった場合は、コード変更作業に入らず必ず回答する。
- 新機能追加時には、MainPage.xaml.cs内に直接実装せず、Helpers/TestMethods.cs内に関数を追加し、MainPage.xaml.csからは1行で委譲する。
- UIの値取得に関するコードはHelpers/UIHelpers.csに集約する。
- グローバルで使用する既定値や定数はMainPage.xaml.cs内の冒頭部に記述する。
- 不明点は必ず確認し、勝手な推測で進めない。
- コード内でのコメントは必ず日本語で記述する。
- コード内に日本語以外でのコメント記述があった場合は日本語に翻訳して記述しなおす。
- ドキュメントに文字化けが含まれる場合は、Shift-JIS/CP932として扱う。ユーザーは可読性のために `docs/handover.md` をBOMなしのUTF-8に変換した。

### 2.3 コミュニケーション
- 低冗長・高シグナル（根拠/結果/次手）。
- 「できる/できない」を先に示し、できない場合は代替案を出す。
- プランやダッシュボードはMarkdown形式で出力する。
...
1. スコープ宣言を最初に固定する
     - 触るのは以下のもののみとする
        ...
        4. 「整理対象」が呼び出すヘルパー関数
        5. 4のヘルパー関数は他からの呼び出しがある場合は整理対象外、無ければ整理対象に含める
        6. 共通化のために新規ヘルパーファイルを追加する場合
        7. .csprojファイルの編集は新規ヘルパーファイルを登録する場合のみ許可する
        8. .csprojファイルに追記が必要なのに追記できない場合はユーザーが手動で行うことを指示する
    - 「可能であれば生成ループ部分自体をヘルパーファイルへ移譲する。」
    - 共通化を行った場合は、整理対象外の関数からも新規ヘルパーファイルを呼び出せるように修正する
    - 共通化を行った場合は、MainPage.xaml.cs等のUI層からも新規ヘルパーファイルを呼び出せるように修正する
    - 共通化を行った場合は、MainPage.xaml.cs内で共通化された機能を把握できるようにコメントを追加する
