...
public sealed class PaperNoise : IDisposable
{
    ...
    public double Sample01Bilinear(double canvasX, double canvasY)
    {
        ...
        // タイル（負値も安全に繰り返す）
        // ここでのcanvasX/canvasYは「ピクセル中心座標（... 0.5, 1.5, 2.5 ...）」として扱う。
        // 呼び出し側が (x + 0.5) を渡す前提なので、サンプル時は -0.5 して texel index と合わせる。
        // これにより、極値（タイルの最深/最浅画素）が双線形で潰れにくくなる。
    ...
    private static bool IsInvalidBackground(SKColor c, InvalidPixelMode mode, SampleChannel sampleChannel)
    {
        ...
        // Alphaチャンネルをノイズ値として使う場合、RGBは無意味なので「透明」を無効背景として扱う。
        // (紙目タイルやdot由来のタイルで、背景がA==0のことが多いため)
